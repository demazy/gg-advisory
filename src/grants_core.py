...
def _extract_response_output_text(data: Dict[str, Any]) -> str:
    """Extract assistant output text from a Responses API response payload."""
    chunks: List[str] = []
...
def _collect_urls_recursive(obj: Any) -> List[str]:
    out: List[str] = []
    if isinstance(obj, dict):
        for k, v in obj.items():
            if k == "url" and isinstance(v, str) and v.startswith(("http://", "https://")):
                ...
                out.extend(_collect_urls_recursive(v))
...
def url_on_allowed_domain(url: str, allowed_domains: Sequence[str]) -> bool:
    d = domain(url)
    allowed = [str(x).lower().removeprefix("www.") for x in allowed_domains]
    ...
    *,
    ...
    for attempt in range(4):
        r = requests.post(
            ...
            headers={"Authorization": f"Bearer {OPENAI_API_KEY}", "Content-Type": "application/json"},
            json=payload,
            timeout=(CONNECT_TIMEOUT, WEB_TIMEOUT),
        )
        if r.status_code < 400:
            break
...
def discover_programmes_via_web(
    *,
    ...
    scope_text: str,
    verified: date,
    known_programmes: Sequence[Dict[str, Any]],
...
def independent_validate_via_web(
    *,
    record: Dict[str, Any],
    allowed_domains: Sequence[str],
    verified: date,
