...
def load_entries(yaml_path: Path, verified: date) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    for e0 in raw.get("grants") or []:
        ...
        # The report tracks current and recurring pathways. Future show_from is always
        # respected; a live explicit status can supersede an old show_until window.
        explicit_status = clean(e.get("status"))
        if sf and verified < sf:
            continue
        if su and verified > su and not explicit_status:
            continue
        e["_status"] = _status(e, verified)
        if e["_status"] == "Archived" and not e.get("include_archived"):
            continue
